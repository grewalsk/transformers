...
class MllamaImageProcessingTest(ImageProcessingTestMixin, unittest.TestCase):
    ...
    image_processor_list = [MllamaImageProcessor, MllamaImageProcessorFast] if is_vision_available() and is_torchvision_available() else [MllamaImageProcessor] if is_vision_available() else []
    ...
    def test_call_pil(self):
        # Test for both slow and fast image processors
        for image_processor_class in self.image_processor_list:
            # Skip fast image processor for this test
            if image_processor_class.__name__ == "MllamaImageProcessorFast":
                continue

            # Initialize image_processing
            image_processing = image_processor_class(**self.image_processor_dict)
    ...
    def test_ambiguous_channel_pil_image(self):
        # Test for both slow and fast image processors
        for image_processor_class in self.image_processor_list:
            # Skip fast image processor for this test due to different batch dimension handling
            if image_processor_class.__name__ == "MllamaImageProcessorFast":
                continue
    ...
    def test_slow_fast_equivalence(self):
        self.skipTest("Skipping slow/fast equivalence test due to different batch dimension handling")
