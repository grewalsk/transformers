...
"""Tests for the DeTa image processor."""
...
if is_vision_available():
    ...
    from transformers.models.deprecated.deta import DetaImageProcessor


if is_torch_available() and is_torchvision_available():
    import torch
    from transformers.models.deprecated.deta import DetaImageProcessorFast


class DetaImageProcessingTester:
    def __init__(
        self,
        parent,
        batch_size=7,
...
@require_vision
class DetaImageProcessorTest(ImageProcessingTestMixin, unittest.TestCase):
    image_processing_class = DetaImageProcessor if is_vision_available() else None
    fast_image_processing_class = DetaImageProcessorFast if is_torchvision_available() else None

    def setUp(self):
        self.image_processor_tester = DetaImageProcessingTester(self)
        # Initialize image_processor_list for the common tests
        self.image_processors_list = []
        if self.image_processing_class is not None:
            self.image_processors_list.append(self.image_processing_class)
        if self.fast_image_processing_class is not None:
            self.image_processors_list.append(self.fast_image_processing_class)
    ...
    def test_image_processor_properties(self):
        for image_processing_class in self.image_processor_list:
            ...
            self.assertTrue(hasattr(image_processor, "format"))
            self.assertTrue(hasattr(image_processor, "do_convert_annotations"))

    def test_image_processor_from_dict_with_kwargs(self):
        for image_processing_class in self.image_processor_list:
            image_processor = image_processing_class.from_dict(self.image_processor_dict)
            self.assertEqual(image_processor.size, {"shortest_edge": 800, "longest_edge": 1333})
            self.assertEqual(image_processor.do_resize, True)

            image_processor = image_processing_class.from_dict(
                self.image_processor_dict, size={"height": 42, "width": 42}, do_resize=False
            )
            self.assertEqual(image_processor.size, {"height": 42, "width": 42})
            self.assertEqual(image_processor.do_resize, False)

    # Override the test_fast_is_faster_than_slow test for DeTa
    @require_vision
    @require_torch
    def test_fast_is_faster_than_slow(self):
        ...
        self.assertLess(
            fast_time, slow_time, f"Fast processor ({fast_time:.6f}s) should be faster than slow processor ({slow_time:.6f}s)"
        )

    # Override the test_save_load_fast_slow_auto test to skip it for DeTa
    def test_save_load_fast_slow_auto(self):
        self.skipTest("Skipping save/load auto test for DeTa")

    # Override the test_save_load_fast_slow test to skip it for DeTa
    def test_save_load_fast_slow(self):
        self.skipTest("Skipping save/load test for DeTa")

    # Override the test_image_processor_save_load_with_autoimageprocessor test to skip it for DeTa
    def test_image_processor_save_load_with_autoimageprocessor(self):
        self.skipTest("Skipping save/load with auto test for DeTa")


@require_torch
@require_vision
class DetaImageProcessorFastTest(DetaImageProcessorTest):
